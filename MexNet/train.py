...
from data_helper import load_w2v, loadSamples, getTopicEmbedding
import os
import time
import datetime

FLAGS=tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer('embedding_size', 100, 'embedding size of words')
tf.app.flags.DEFINE_string('word_embedding_file', 'zhihu/data/ieee_zhihu_cup/w2v_100.model', 'word embedding file')
...
tf.app.flags.DEFINE_string('topic_info', 'zhihu/data/ieee_zhihu_cup/topic_info.txt', 'topic info')
...
tf.app.flags.DEFINE_integer('decay_steps', 3000, 'decay steps')
tf.app.flags.DEFINE_float('decay_rate', 0.85, 'decay rate')
tf.app.flags.DEFINE_integer('rnn_hidden_size', 128, 'batch_size')
...
tf.app.flags.DEFINE_float('l1_dropout_keep_prob', 1.0, 'l1_dropout_keep_prob')
tf.app.flags.DEFINE_float('l2_dropout_keep_prob', 1.0, 'l2_dropout_keep_prob')
tf.app.flags.DEFINE_integer('fc_layer_size', 2048, 'fc_layer_size')
...
def main(_):
    ...
    train_x, train_title, train_y, valid_x, valid_title, valid_y, label_map = loadSamples(FLAGS.training_data, FLAGS.label_file, FLAGS.label_map, FLAGS.eval_data_file, word2id, FLAGS.valid_rate, FLAGS.num_classes, FLAGS.sent_len, FLAGS.sent_len, FLAGS.doc_len)
    assert(len(train_x) == len(train_title) and len(train_x) == len(train_y))
    print('Load train/valid samples finished')

    mem = getTopicEmbedding(FLAGS.topic_info, w2v_model, word2id, label_map, FLAGS.embedding_size)
    print('mem', mem.shape)
        ...
        with sess.as_default(), tf.device('/gpu:0'):
            ...
            for epoch in range(0, FLAGS.num_epochs):
                ...
                for start, end in zip(range(0, train_sample_size, FLAGS.batch_size), range(FLAGS.batch_size, train_sample_size, FLAGS.batch_size)):
                    ...
                    batch_input_x, mask, batch_input_title, title_mask = paddingX(batch_input_x, batch_input_title, FLAGS.sent_len, FLAGS.sent_len, FLAGS.doc_len)
                    ...
                    feed_dict = {
                        ...
                        mem_net.l1_dropout_keep_prob: FLAGS.l1_dropout_keep_prob,
                        mem_net.l2_dropout_keep_prob: FLAGS.l2_dropout_keep_prob
                    }
                    loss, acc, step, summaries, _ = sess.run([mem_net.loss_val, mem_net.accuracy, mem_net.global_step, train_summary_op, mem_net.train_op], feed_dict)
                    ...
                    if batch_step % FLAGS.evaluate_every == 0 and total_step > 0:
                        ...
                        for start, end in zip(range(0, dev_sample_size, FLAGS.batch_size), range(FLAGS.batch_size, dev_sample_size, FLAGS.batch_size)):
                            ...
                            step, summaries, loss, acc, logits = sess.run([mem_net.global_step, dev_summary_op, mem_net.loss_val, mem_net.accuracy, mem_net.logits], feed_dict)
...
def paddingX(input_x, input_title, title_len, sent_len, doc_len):
    new_x = []
    ...
    new_title = []
    tmp = [0 for i in range(title_len)]
    for t in input_title:
        new_title.append(t[:] + tmp[len(t):title_len])
    new_title = np.array(new_title)
    title_mask = (new_title > 0).astype(np.float32)
    return (new_x, mask, new_title, title_mask)
