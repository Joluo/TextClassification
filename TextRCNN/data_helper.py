...
def loadSamples(question_train_set, question_topic_train_set, label_map_file_path, word2id, valid_rate, num_classes, title_only = False):
    ...
    for line in open(question_train_set).readlines():
        ...
        samples_len.append(len(sample_x))
    samples_x = np.array(samples_x)
    samples_y = np.array(samples_y)
    samples_len = np.array(samples_len)
    sample_size = len(samples_x)
    train_sample_size = int(sample_size * (1 - valid_rate))
    return (samples_x[:train_sample_size], samples_y[:train_sample_size], samples_len[:train_sample_size], samples_x[train_sample_size:], samples_y[train_sample_size:], samples_len[train_sample_size:])

def loadEvalSample(eval_sample_path, word2id):
    eval_samples = list()
    eval_samples_len = list()
    for line in open(eval_sample_path).readlines():
        ...
        eval_samples_len.append(len(sample_ids))
    eval_samples = np.array(eval_samples)
    eval_samples_len = np.array(eval_samples_len)
    return (eval_samples, eval_samples_len)
