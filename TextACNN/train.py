...
tf.app.flags.DEFINE_integer('embedding_size', 200, 'embedding size of words')
...
tf.app.flags.DEFINE_integer('eval_words_threshold', 1, 'eval_words_threshold')
...
tf.app.flags.DEFINE_string('filter_sizes', '1,2,3,4,5,6,7', 'filter sizes')
tf.app.flags.DEFINE_integer('num_filters', 512, 'number of filters')
tf.app.flags.DEFINE_float('l2_reg_lambda', 0.0001, 'L2 regular para')
...
def main(_):
    ...
    w2v_model, word2id = load_w2v(FLAGS.word_embedding_file, FLAGS.embedding_size, FLAGS.topic_info, FLAGS.raw_eval_file, FLAGS.eval_words_threshold)
            ...
            for epoch in range(0, FLAGS.num_epochs):
                ...
                for start, end in zip(range(0, train_sample_size, FLAGS.batch_size), range(FLAGS.batch_size, train_sample_size, FLAGS.batch_size)):
                    ...
                    feed_dict = {
                        ...
                        text_cnn.phase_train: True
                    }
                    if True: #batch_step % FLAGS.print_stats_every == 0:
                        ...
                        batch_acc += acc
                    #else:
                    #    _ = sess.run([text_cnn.train_op], feed_dict)
                    batch_step += 1
                    total_step += 1.
                    ...
                    if batch_step % FLAGS.evaluate_every == 0 and total_step > 80000:
                        eval_loss = 0.
                        eval_acc = 0.
                        eval_step = 0
                        for start, end in zip(range(0, dev_sample_size, FLAGS.batch_size), range(FLAGS.batch_size, dev_sample_size, FLAGS.batch_size)):
                            ...
                            zhihuStats(logits, batch_input_y)
                            eval_loss += loss
                            eval_acc += acc
                            eval_step += 1
                        ...
                        #dev_summary_writer.add_summary(summaries, step)
                    if batch_step % FLAGS.checkpoint_every == 0 and total_step > 80000:
